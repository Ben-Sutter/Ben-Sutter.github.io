//Source, HelloPhong.js from the HelloPhong example on the CS 418 course website
...
// View parameters
var eyePt = glMatrix.vec3.fromValues(0.0,0.0,2.0);
...
function draw() { 
    ...
    for (var i = 0; i < particles.length; i++) {
        ...
        if (currentParticle.dead > 500) {
            continue;
        } else {
            currentParticle.updateParticleInfo();
            //glMatrix.vec3.set(transformVec,20,20,20);
            glMatrix.mat4.translate(mvMatrix, mvMatrix, currentParticle.position);
            glMatrix.mat4.scale(mvMatrix, mvMatrix, currentParticle.radius);

            //Get material color
            //colorVal = "#ffff00";
            R = currentParticle.color[0];
            G = currentParticle.color[1];
            B = currentParticle.color[2];
 ...
 */
 function startup() {
  ...
  setPhongShader();
  setupBuffers();
  document.getElementById("addP").addEventListener("click", addParticle);
     document.getElementById("removeP").addEventListener("click", removeAllParticles);
